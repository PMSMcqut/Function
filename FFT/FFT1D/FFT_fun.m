...
switch type
    case '1D'
        switch method
            ...
            case 'gui'
                FFT_signal=struct('time',[],'blockName','data_yanglu');
                FFT_signal.signals=struct('values',[],'dimensions',1,'label','','title','','plotStyle',[1,1]);
                FFT_signal.time = time;%[0:1e-6:(length(data)-1)*1e-06]'; %由采样率和总时间长度决定
                ...
                Fourier.P.Order=Fourier.freq/f0;
                Fourier.P.Amplitude=Fourier.mag;
                Fourier.P.Phase=Fourier.phase;
                Fourier.P.Frequency=Fourier.freq;              
        end
    case '2D'
        %% 信号FFT分析
