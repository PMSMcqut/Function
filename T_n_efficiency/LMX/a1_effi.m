...
%% 绘图程序
aa_nz_a1=a_nz;aa_torque_a1=T;aa_efficiency_Pmax_a1=effi;%为了画图方便重置变量名称
%% 绘制效率MAP
figure(1);
[C,h]=contour(aa_nz_a1, aa_torque_a1, aa_efficiency_Pmax_a1,50);
axis([0 12000 14 350]);%设置坐标轴刻度
colormap(jet);
hold on
plot(n1,Tmax,'b','Linewidth',2);
figure(2);
gca=pcolor(aa_nz_a1, aa_torque_a1, aa_efficiency_Pmax_a1);
axis([0 12000 14 350]);
colorbar;
colormap(jet);
hold on
plot(n1,Tmax,'b','Linewidth',2);
figure(3);
[C,h]=contourf(aa_nz_a1, aa_torque_a1, aa_efficiency_Pmax_a1,50);
axis([0 12000 14 350]);
colorbar;
colormap(jet);%设置图形颜色
...
hold on
plot(n1,Tmax,'b','Linewidth',2);
saveas(gcf,'efficiency_map_a1');
%% 存储数据
save('a1.mat','aa_nz_a1','aa_torque_a1','aa_efficiency_Pmax_a1');%存储数据，记得不同的并联之路要修改文件名
