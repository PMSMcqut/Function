...
%% 绘图程序
aa_nz_a4=a_nz;aa_torque_a4=T;aa_efficiency_Pmax_a4=effi;%为了画图方便重置变量名称
%% 绘制T-n曲线
figure(1)
plot(n1,Tmax);
%% 绘制效率MAP
figure(1);
[C,h]=contour(aa_nz_a4, aa_torque_a4, aa_efficiency_Pmax_a4,50);
axis([0 24000 4 85]);%设置坐标轴刻度
colormap(jet);
hold on
plot(n1,Tmax,'b','Linewidth',2);
figure(2);
gca=pcolor(aa_nz_a4, aa_torque_a4, aa_efficiency_Pmax_a4);
axis([0 24000 4 85]);
colorbar;
colormap(jet);
hold on
plot(n1,Tmax,'b','Linewidth',2);
figure(3);
[C,h]=contourf(aa_nz_a4, aa_torque_a4, aa_efficiency_Pmax_a4,50);
axis([0 24000 4 85]);
colorbar;
colormap(jet);%设置图形颜色
...
hold on
plot(n1,Tmax,'b','Linewidth',2);
saveas(gcf,'efficiency_map_a4');
save('a4.mat','aa_nz_a4','aa_torque_a4','aa_efficiency_Pmax_a4');%存储数据，记得不同的并联之路要修改文件名
